...
Package \texttt{istp} supports the importance sampling with transparent reparameterization and convenience priors on an example of saturated logistic regression with incomplete outcome data introduced in [@lee-2024-issues].

The data has three components: the binary response variable $Y$, the binary independent variable $\mathbf{X}  = (X_1, \dots, X_p)^T$, and the indicator variable $R$, which indicates the missingness of $Y$. The binary variable $R$ is 1 when $Y$ is observed and $0$ when $Y$ is unobserved.
